...
Este sistema simula um fluxo de cadastro e validação de informações estudantis. Ele faz uso estrito de estruturas de controle de fluxo, manipulação avançada de strings e tratamento iterativo de entradas via `Scanner`, garantindo a integridade dos dados inseridos pelo usuário.
...
* **Cálculo Aproximado de Idade:** Processamento de substrings de datas no formato `dd/mm/aaaa` para estimar a idade do usuário com base em uma data de referência fixa.
...
Biblioteca Nativa:
 ...
 ```java.util.Scanner ``` — Entrada de dados via terminal e controle de fluxo iterativo
